...
epi_head_and_tail(test_df[test_df$d_time_cuts_DEF_pre == 'pre-T0', ], cols = 4)
epi_head_and_tail(test_df[test_df$d_time_cuts_DEF_post == 'pre-T0', ], cols = 4)
# but there are no empty(NA) rows:
test_df[rowSums(is.na(test_df)) == ncol(test_df), ] # should be empty
subset_preT0 <- test_df[test_df$d_time_cuts_DEF_post == 'pre-T0', ]
...
epi_head_and_tail(df_j[df_j$d_time_cuts_DEF == 'pre-T0', ], cols = 6)
epi_head_and_tail(df_j[df_j$d_time_cuts_DEF_pre == 'pre-T0', ], cols = 6)
epi_head_and_tail(df_j[df_j$d_time_cuts_DEF_post == 'pre-T0', ], cols = 6)
epi_head_and_tail(df_j[df_j$d_time_cuts_DEF_post == 'pre-T0', ], last_cols = TRUE)


epi_head_and_tail(df_recode[df_recode$d_time_cuts_DEF == 'pre-T0', ], cols = 6)

View(tail(df[df$d_time_cuts_DEF == 'pre-T0', ], n = 100))
